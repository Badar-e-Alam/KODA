"""Customers Http Handlers module.
...
class CustomersConfig:
    """Knobs the customers module reads at startup."""
    enabled: bool = True
    timeout_s: float = 5.0
    retries: int = 3
...
class Customers:
    ...
    ``customers(...)`` defined at module scope.
    """

    def __init__(self, config: CustomersConfig | None = None) -> None:
        self.config = config or CustomersConfig()
...
def customers(payload: dict[str, Any]) -> dict[str, Any]:
    """Module-level convenience wrapper around the default Customers instance."""
