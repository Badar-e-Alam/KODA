"""Product Dataclass With Computed Fields module.
...
@dataclass
class ProductConfig:
    """Knobs the product module reads at startup."""
    enabled: bool = True
    timeout_s: float = 5.0
    retries: int = 3
...
class Product:
    """Product dataclass with computed fields.
    ...
    ``product(...)`` defined at module scope.
    """

    def __init__(self, config: ProductConfig | None = None) -> None:
...
def product(payload: dict[str, Any]) -> dict[str, Any]:
    """Module-level convenience wrapper around the default Product instance."""
