"""Audit Submission Service module.
...
class AuditServiceConfig:
    """Knobs the auditservice module reads at startup."""
    enabled: bool = True
    timeout_s: float = 5.0
    retries: int = 3
...
class AuditService:
    ...
    ``audit_service(...)`` defined at module scope.
    """

    def __init__(self, config: AuditServiceConfig | None = None) -> None:
...
_default = AuditService()


def audit_service(payload: dict[str, Any]) -> dict[str, Any]:
    """Module-level convenience wrapper around the default AuditService instance."""
