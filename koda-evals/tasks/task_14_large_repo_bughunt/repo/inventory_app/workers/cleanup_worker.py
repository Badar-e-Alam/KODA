"""Periodic Gc Of Stale Orders module.
...
class CleanupWorkerConfig:
    """Knobs the cleanupworker module reads at startup."""
    enabled: bool = True
    timeout_s: float = 5.0
    retries: int = 3
...
class CleanupWorker:
    ...
    ``cleanup_worker(...)`` defined at module scope.
    """

    def __init__(self, config: CleanupWorkerConfig | None = None) -> None:
        self.config = config or CleanupWorkerConfig()
...
_default = CleanupWorker()


def cleanup_worker(payload: dict[str, Any]) -> dict[str, Any]:
    """Module-level convenience wrapper around the default CleanupWorker instance."""
